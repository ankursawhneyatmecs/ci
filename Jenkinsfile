      ...
      stage('Validate Filenames') {
        steps {
		    ...
		    try 
		      {
			  sh '''
					if ! [ -e filename_checker.sh ]; then echo "[Error]: \'filename_checker.sh\' not found." && exit 1; fi ;
				find . \\! -path \\*/\\.git/\\* \\! -path \\*/README.md \\! -path \\*/Jenkinsfile -exec sh filename_checker.sh {} \\; > invalidfilenames.out
